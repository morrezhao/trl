    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        if (
            ...
            if self.args.report_to and "tensorboard" in self.args.report_to:
                # Trainer 里会懒加载一个 tb_writer，我们直接拿来用
                tb_writer = getattr(self, "tb_writer", None)
                if tb_writer is None and hasattr(self, "_get_tb_writer"):
                    tb_writer = self._get_tb_writer()
                    self.tb_writer = tb_writer

                if tb_writer is not None:
                    # 可选：去重 + 随机采样，行为和 wandb 一致
                    ...
                    if self.wandb_log_unique_prompts:
                        ...
                        prompts, completions = new_prompts, new_completions

                    if self.num_completions_to_print and len(prompts) > self.num_completions_to_print:
                        # 简单随机采样
                        indices = random.sample(range(len(prompts)), self.num_completions_to_print)
                        prompts = [prompts[i] for i in indices]
                        completions = [completions[i] for i in indices]

                    # 以 text summary 的形式写入 TensorBoard
                    # 1）汇总成一个大文本
                    all_text = []
                    for idx, (p, c) in enumerate(zip(prompts, completions)):
                        all_text.append(f"### Sample {idx}\n**Prompt:**\n{p}\n\n**Completion:**\n{c}\n")
                    if all_text:
                        tb_writer.add_text(
                            "completions",
                            "\n\n---\n\n".join(all_text),
