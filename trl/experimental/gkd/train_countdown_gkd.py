...
"""
GKD (Generalized Knowledge Distillation) training script for Countdown Task.

Reproduces the GOLD paper experiment on Countdown-Task with:
- Student: Qwen/Qwen2.5-1.5B-Instruct
- Teacher: Qwen/Qwen3-4B-Instruct-2507

This script trains the student model using on-policy distillation and tracks
Pass Rate during training to monitor reasoning capability improvement.

Usage:
    # Single GPU
    python train_countdown_gkd.py

    # Multi-GPU with DeepSpeed
    accelerate launch --config_file configs/accelerate_deepspeed.yaml train_countdown_gkd.py
"""
...
from countdown_eval import verify_countdown_response
...
USER_PROMPT_TEMPLATE = (
    "Using all the numbers in {nums_list}, create an equation that equals {target}. "
    "You can use basic arithmetic operations (+, -, *, /) and each number can only be used once. "
    "Show your work in <think> </think> tags. "
    "And return the final equation and answer in <answer> </answer> tags, "
    "for example <answer> (1 + 2) / 3 = 1 </answer>."
)

def format_countdown_to_messages(example: dict) -> dict:
    nums_list_str = str(example["nums"])  # e.g. "[5, 38, 85, 26]"
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {
            "role": "user",
            "content": USER_PROMPT_TEMPLATE.format(nums_list=nums_list_str, target=example["target"]),
        },
        {"role": "assistant", "content": ""},
    ]
    return {"messages": messages, "target": example["target"], "nums": example["nums"]}
    ...
    def _evaluate_pass_rate(self, model, step: int = 0, batch_size: int = 8) -> float:
        ...
        with torch.no_grad():
            for batch_idx in iterator:
                ...
                batch_nums = all_nums[start_idx:end_idx]
                ...
                for i, (output, target, nums) in enumerate(zip(outputs, batch_targets, batch_nums)):
                    ...
                    is_correct = verify_countdown_response(response, target, nums)
                    if is_correct:
                        correct += 1
                    total += 1
...
def main():
    ...
    # Following GOLD paper settings for Countdown task
    training_args = GKDConfig(
        output_dir="./qwen-1.5b-countdown-gkd",
        # GKD specific parameters
        # beta=0.5: Balanced Jensen-Shannon Divergence (paper default)
        # beta=0.0: Forward KL (mode-seeking)
        # beta=1.0: Reverse KL (mean-seeking)
        beta=0.0,
        # Lambda controls on-policy ratio
        # lmbda=1.0: Pure on-policy (student generates all completions)
        ...
        # Model loading kwargs - enable flash attention
        # model_init_kwargs={
        ...
        run_name="qwen-1.5b-countdown-gkd",
    )
    ...
    # Load Countdown dataset (only on main process, then broadcast)
    main_print("Loading Countdown-Tasks dataset...")
    if is_main_process():
        ...
        formatted_dataset = dataset["train"].map(
            format_countdown_to_messages,
            desc="Formatting dataset",
        )

        # Split into train and eval
        # Use a small subset for faster experimentation
        train_size = 10000  # Adjust based on available compute
        eval_size = 50

        train_dataset = formatted_dataset.shuffle(seed=42).select(range(train_size))
        eval_dataset = formatted_dataset.shuffle(seed=43).select(
    ...
    pass_rate_callback = PassRateEvaluationCallback(
        ...
        num_eval_samples=50,  # Use 50 samples for evaluation
        max_new_tokens=512,
    )
